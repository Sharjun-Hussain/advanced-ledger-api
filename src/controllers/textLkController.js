...
const { Setting, Shop, Customer, TextLkTemplate, TextLkCampaign } = require('../models');
...
const getConfig = async (req, res, next) => {
    try {
        ...
        const org = await Shop.findByPk(req.user.shop_id, {
...
const testConnection = async (req, res, next) => {
    try {
        ...
        if (apiKey === MASK) {
            const setting = await Setting.findOne({
                where: { shop_id: req.user.shop_id, category: 'textlk_crm' }
            });
            if (setting) {
...
const getContacts = async (req, res, next) => {
    try {
        ...
        const contacts = await textLkService.getContacts(req.user.shop_id, page, limit);
...
const createContactGroup = async (req, res, next) => {
    try {
        ...
        const result = await textLkService.createGroup(req.user.shop_id, name);
...
const updateContactGroup = async (req, res, next) => {
    try {
        ...
        const result = await textLkService.updateGroup(req.user.shop_id, uid, name);
...
const deleteContactGroup = async (req, res, next) => {
    try {
        ...
        const result = await textLkService.deleteGroup(req.user.shop_id, uid);
...
const sendSms = async (req, res, next) => {
    try {
        ...
        const result = await textLkService.sendSms(req.user.shop_id, { recipient, message, template_id });
...
const syncCustomers = async (req, res, next) => {
    try {
        ...
        const customers = await Customer.findAll({
            where: { shop_id: req.user.shop_id, is_active: true }
        });
        ...
        const groupsRes = await textLkService.getGroups(req.user.shop_id);
        const groups = groupsRes.data || [];
        let posGroup = groups.find(g => g.name === 'POS Customers');

        if (!posGroup) {
            const newGroup = await textLkService.createGroup(req.user.shop_id, 'POS Customers');
        ...
        for (const customer of customers) {
            try {
                ...
                await textLkService.createContact(req.user.shop_id, {
...
const createCampaign = async (req, res, next) => {
    try {
        ...
        // 2. Send to Text.lk
        try {
            const result = await textLkService.sendCampaign(req.user.shop_id, {
                contact_list_id,
                message,
                dlt_template_id,
                schedule_time
...
const getStats = async (req, res, next) => {
    try {
        ...
        try {
            const balanceData = await textLkService.getBalance(req.user.shop_id);
        ...
        try {
            const smsLogsRes = await textLkService.getSmsLogs(req.user.shop_id);
